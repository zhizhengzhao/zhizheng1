...
'''
s2: Full KAN (cubic B-spline) — CLS block FFN only.
Same replacement scope as v2, but using order-3 B-spline basis instead of
...
def get_model(data_config, **kwargs):
    ...
    cfg = dict(
        ...
        fc_params=[],
        activation='gelu',
        # KAN spline switches — same scope as v2 (CLS blocks only)
        use_kan_head=False,
    ...
    _logger.info('Model config (s2 — spline CLS blocks): %s' % str(cfg))
