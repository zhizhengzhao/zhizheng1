...
'''
s1: Full KAN (cubic B-spline) — classification head only.
Same replacement scope as v1, but using order-3 B-spline basis instead of
...
def get_model(data_config, **kwargs):
    ...
    cfg = dict(
        ...
        fc_params=[],
        activation='gelu',
        # KAN spline switches — same scope as v1 (head only)
        use_kan_head=True,
    ...
    _logger.info('Model config (s1 — spline head): %s' % str(cfg))
