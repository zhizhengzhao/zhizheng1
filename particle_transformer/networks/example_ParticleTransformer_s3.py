...
'''
s3: Full KAN (cubic B-spline) — main block FFN only.
Same replacement scope as v3, but using order-3 B-spline basis instead of
...
def get_model(data_config, **kwargs):
    ...
    cfg = dict(
        ...
        fc_params=[],
        activation='gelu',
        # KAN spline switches — same scope as v3 (main blocks only)
        use_kan_head=False,
    ...
    _logger.info('Model config (s3 — spline main blocks): %s' % str(cfg))
