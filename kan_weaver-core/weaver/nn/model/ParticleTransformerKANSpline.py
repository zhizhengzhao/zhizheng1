...
# ─── KAN ──────────────────────────────────────────────────────
from weaver.nn.model.kan_spline_layers import KANSplineLinear, KANSplineClassificationHead
...
class Block(nn.Module):
    def __init__(self, embed_dim=128, num_heads=8, ffn_ratio=4,
        ...
        # ─── KAN Spline ───────────────────────────────────────────
        if self.use_kan_ffn:
            self.fc1 = KANSplineLinear(
                in_features=embed_dim,
                out_features=self.ffn_dim,
                num_grids=kan_ffn_num_grids,
                spline_order=kan_ffn_spline_order,
                grid_range=kan_ffn_grid_range,
                base_activation=kan_ffn_base_activation,
                grid_eps=kan_ffn_grid_eps)
        else:
        # ─── KAN Spline End ──────────────────────────────────────
...
class ParticleTransformer(nn.Module):
    ...
    def __init__(self,
        ...
        cfg_block.setdefault('kan_ffn_spline_order', kan_ffn_spline_order)
        ...
        cfg_block.setdefault('kan_ffn_grid_eps', kan_ffn_grid_eps)
        # ─── KAN Spline End ──────────────────────────────────────
        ...
        cfg_cls_block.setdefault('kan_ffn_spline_order', kan_ffn_spline_order)
        cfg_cls_block.setdefault('kan_ffn_grid_range', kan_ffn_grid_range)
        cfg_cls_block.setdefault('kan_ffn_base_activation', kan_ffn_base_activation)
        cfg_cls_block.setdefault('kan_ffn_grid_eps', kan_ffn_grid_eps)
        _logger.info(
            'KAN Spline FFN: main=%s, cls=%s, grids=%s, order=%s, range=%s, act=%s',
            cfg_block['use_kan_ffn'],
            cfg_cls_block['use_kan_ffn'],
            kan_ffn_num_grids,
            kan_ffn_spline_order,
        ...
        if fc_params is not None:
            ...
            if use_kan_head:
                _logger.info(
                    'Using KAN Spline head: grids=%s, order=%s, range=%s, act=%s',
                    kan_head_num_grids, kan_head_spline_order,
                    str(kan_head_grid_range), kan_head_base_activation)
                self.fc = KANSplineClassificationHead(
                    input_dim=embed_dim,
                    num_classes=num_classes,
                    fc_params=fc_params,
                    num_grids=kan_head_num_grids,
                    spline_order=kan_head_spline_order,
                    grid_range=kan_head_grid_range,
                    base_activation=kan_head_base_activation,
                    grid_eps=kan_head_grid_eps)
            else:
            # ─── KAN Spline End ──────────────────────────────────────
                fcs = []
                in_dim = embed_dim
